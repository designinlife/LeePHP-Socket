...
use LeePHP\OptionKit\OptionPrinterInterface;
use \Iterator;
use \Exception;

class OptionSpecCollection implements Iterator {
    public $data         = array();
    ...
    function add() {
        ...
        if (is_object($first) && is_a($first, __NAMESPACE__ . '\OptionSpec')) {
            $this->addSpec($first);
    ...
    function outputOptions($class = NULL, $width = 24) {
        if (!$class)
            $class = __NAMESPACE__ . '\OptionPrinter';
        
        $printer = new $class($this);
        if (!( $printer instanceof OptionPrinterInterface )) {
